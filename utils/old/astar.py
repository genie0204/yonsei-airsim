...
class Astar():
    def __init__(self, json_file='./airsim_nh.json'):
    ...
    def get_random_start_end_points(self, start=True, neighbour=False):
        if start == True:
            ...
            _temp_end_point = self.start_point
        ...
        if start == True:
            ...
            while True:
                ...
                self.end_point = {
                    'name': -1,
                    'pos_x': _temp_end_point['pos_x'],
                    'pos_y': _temp_end_point['pos_y'],
                    'connected_nodes': _temp_end_point['connected_nodes']
                }

                for i, n in enumerate(self.nodes):
                    if n['name'] in _temp_end_point['connected_nodes'] or n['name'] in _temp_end_point['connected_nodes']:
                        n['connected_nodes'].append(-1)
                        self.nodes[i] = n
    ...
    def computeAdversarialPath(self):
        
        while True:
            while True:
                try:
                    self.get_random_start_end_points(neighbour=True)
                    break
                except:
                    print(1)
            while True:
            ...
            _rs_point = self.coords[0]

            # When car is located near the corner, compute again.
            _rstart_direction = self.get_start_direction(self.coords)
            self.coords.pop(0)
            _rcoords = self.coords

            if len(self.coords) > 50 and len(self.coords) < 150:
                if _start_direction is not None:
                    break    
        
        return _s_point, _start_direction, _coords, _rs_point, _rstart_direction, _rcoords
